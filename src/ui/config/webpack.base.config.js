 ...
 * @LastEditors  : VSCode
 * @LastEditTime : 2019-12-25 09:52:36
...
const { resolve, hasPackagePlugin } = utils;
	...
	module: {
		rules: [
			<%if(eslint.value) {%>
			{
				...
				options: {
					cache: true,
					fix: <%= eslint.lintOn && eslint.lintOn.includes('save') %>,
			...
			...utils.styleLoaders({
				...
				// 注入sass配置
				<%if(['sass', 'dart-sass', 'node-sass'].includes(cssPreprocessors)) {%>
